...
4. 配置真实 `FITNESS_PUBLIC_ORIGIN=https://实际域名` 和对应 `FITNESS_ALLOWED_HOSTS=实际域名`。保留 `shared`、安全 Cookie；注册与游客先关闭，准备好公开体验后可分别设为 `true`。首次保留禁用 AI 与零额度，验证账号和记录不需要生成费用。模型 Key 以后只在服务器编辑器中填入，不粘贴到 GitHub、聊天记录或命令行参数中。
...
6. 把 `DEPLOY_ENABLED` 改为 `true`，在 GitHub Actions 手动运行“检查与发布”，选择 `main`。检查成功后会创建或推进 `deploy`。记录这次的完整提交 SHA，再在 ECS 执行第一次部署：
   ...
   按提示输入用户名和两次密码，密码输入不显示；不要把密码写在命令参数、管道、聊天或仓库中。每名试用者建立独立账号；也可在配置中明确开放网页注册。此工具只操作已存在的数据库，不会替你初始化数据库或重置已有账号。
...
注册与游客版本增加v14数据表，已有v13服务不能直接自动更新。先按[账号升级说明](../docs/ACCOUNTS.md)安排停服备份、明确迁移和候选版本启用。固定安装的check_release.py也须经管理员审核更新，旧检查要求注册关闭。不要将本次结构升级当作普通同结构代码回退；部署预检保护没有删除。
