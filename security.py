...
def issue_session(request: Request, response: Response, user_id: int):
    ...
    with request.app.state.database.connect() as connection:
        ...
        connection.execute(
            ...
            (token_hash(token), user_id, int(time.time()) + settings.session_seconds),
        )
    response.set_cookie(
        COOKIE_NAME, token, max_age=settings.session_seconds, httponly=True,
